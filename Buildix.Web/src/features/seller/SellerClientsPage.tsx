...
const PAGE_SIZE = 50;
const GRID = 'grid-cols-[1.8fr_1.2fr_1fr]';
...
export default function SellerClientsPage() {
  ...
  const listQuery = useQuery({
    queryKey: ['seller-clients', { search: debouncedSearch }],
    queryFn: () => customersApi.listPaged({ page: 1, size: PAGE_SIZE, search: debouncedSearch }),
      ...
      <div className="mx-auto flex w-full max-w-[1240px] flex-1 flex-col gap-[18px] p-8">
        ...
        <Card className="overflow-hidden">
          ...
          {listQuery.isLoading ? (
            ...
            rows.map((c) => <Row key={c.id} customer={c} />)
          ) : (
