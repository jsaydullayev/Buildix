...
namespace Buildix.Application.Validation;
...
public sealed class StrongPasswordAttribute : ValidationAttribute
{
    ...
    /// <summary>
    /// Programmatic policy check for service-layer paths that set a password
    /// outside model validation (SuperAdmin owner-creation etc.). Unlike
    /// <see cref="IsValid"/> — which treats an omitted value as "leave
    /// unchanged" — this requires a real, policy-strong value: null / empty
    /// returns <c>false</c>. Single-sources the rule so no service re-implements
    /// a divergent inline length check.
    /// </summary>
