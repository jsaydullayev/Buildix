...
using CommonDTOs = Buildix.Application.DTOs;
...
public class MarketService : IMarketService
{
    ...
    public MarketService(IUnitOfWork unitOfWork, ILogger<MarketService> logger, IAppDbContext context, IJwtService jwtService)
    {
    ...
    public async Task<RegisterMarketResponse> RegisterMarketForOwnerAsync(RegisterMarketRequest request, Guid ownerId, CancellationToken cancellationToken = default)
    {
        ...
        // The owner's tenancy just changed, so any JWT they still hold (minted
        // before they had a market) is stale. Mint a fresh one from the in-memory
        // owner entity — its MarketId was set inside the transaction above, so no
        // reload is needed. This used to live in the controller after a redundant
        // GetByIdAsync round-trip.
        var token = _jwtService.GenerateToken(owner, populateExp: true);

        return new RegisterMarketResponse(marketDto, ownerDto, token.AccessToken);
    }
    ...
    public async Task<PublicMarketStateDto?> GetPublicStateBySubdomainAsync(string subdomain, CancellationToken cancellationToken = default)
    {
        var slug = subdomain?.Trim().ToLowerInvariant();
        ...
        // Unknown or soft-deleted storefront → 404 (the caller maps null to
        // NotFound). We never distinguish "no such slug" from "deactivated" —
        // both are simply "this door doesn't open".
        if (market is null || !market.IsActive) return null;
        ...
        return new PublicMarketStateDto(slug, market.Name, state, market.ExpiresAt);
    }
}
