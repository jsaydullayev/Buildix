...
using Buildix.Infrastructure.Services;
...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("Application", "Buildix")
    .WriteTo.Console(
...
try
{
    ...
    Log.Information("Starting Buildix API");
    ...
    builder.Services.AddDbContext<AppDbContext>(options =>
    {
        options.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection"),
            npgsqlOptions =>
            {
                ...
                npgsqlOptions.MigrationsAssembly("Buildix.Infrastructure");
            });
    ...
    builder.Services.AddScoped<Buildix.Application.Interfaces.IAppDbContext>(
    ...
    builder.Services.AddSingleton<IAuthorizationPolicyProvider, Buildix.API.Authorization.PermissionPolicyProvider>();
    builder.Services.AddScoped<IAuthorizationHandler, Buildix.API.Authorization.PermissionAuthorizationHandler>();
    ...
    builder.Services.AddRateLimiter(options =>
    {
        ...
        // /api/public/market/{slug} — anonymous, hit on every login-page load.
        // Read-only single-row lookup, so more generous than registration-submit
        // but still per-IP capped to blunt slug-enumeration scanners.
        options.AddPolicy("public-market", ctx => System.Threading.RateLimiting.RateLimitPartition.GetSlidingWindowLimiter(
            PartitionKey(ctx),
            _ => new System.Threading.RateLimiting.SlidingWindowRateLimiterOptions
            {
                PermitLimit = 30,
    ...
    builder.Services.AddControllers()
        .AddJsonOptions(options =>
        {
            ...
            options.JsonSerializerOptions.Converters.Add(new Buildix.API.Serialization.TashkentTimeJsonConverter());
            options.JsonSerializerOptions.Converters.Add(new Buildix.API.Serialization.TashkentTimeJsonConverterNullable());
        });
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        options.SwaggerDoc("v1", new OpenApiInfo
        {
            Title = "Buildix API",
            Version = "v1",
            Description = "Buildix API with Sales, Inventory, and Debt Tracking"
        });
    ...
    builder.Services.AddScoped<IZakupRoleShaper, ZakupRoleShaper>();
    builder.Services.AddScoped<ISupplierService, SupplierService>();
    // Report layer — the former 2700-line ReportService has been decomposed into
    // focused services (Reports/*). The facade + IReportService are gone; the
    // controllers inject these focused interfaces directly.
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.IDashboardService, Buildix.Application.Services.Reports.DashboardService>();
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.ISalesReportService, Buildix.Application.Services.Reports.SalesReportService>();
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.IFinancialReportService, Buildix.Application.Services.Reports.FinancialReportService>();
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.ISalesListService, Buildix.Application.Services.Reports.SalesListService>();
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.IReportPdfExportService, Buildix.Application.Services.Reports.ReportPdfExportService>();
    builder.Services.AddScoped<Buildix.Application.Interfaces.Reports.IReportExcelExportService, Buildix.Application.Services.Reports.ReportExcelExportService>();
    ...
    // Idempotency-Key store for money-moving endpoints (opt-in via header).
    builder.Services.AddScoped<IIdempotencyService, Buildix.Infrastructure.Services.IdempotencyService>();
    builder.Services.AddScoped<IMarketService, MarketService>();
    builder.Services.AddScoped<IMarketSettingsService, MarketSettingsService>();
    // Telegram notifications — real Bot API client, token from Telegram:BotToken.
    ...
    builder.Services.AddScoped<ICustomersExcelExportService, CustomersExcelExportService>();
    builder.Services.AddScoped<IDebtsExcelExportService, DebtsExcelExportService>();
    builder.Services.AddScoped<IProductCategoriesExcelExportService, ProductCategoriesExcelExportService>();
    builder.Services.AddScoped<IZakupsExcelExportService, ZakupsExcelExportService>();
    builder.Services.AddScoped<ISuppliersExcelExportService, SuppliersExcelExportService>();
    // Image-upload parsing (multipart/JSON → validated bytes) — web-layer plumbing
    // lifted out of ProductsController/UsersController so those actions stay thin.
    ...
    await Buildix.API.Bootstrap.SuperAdminSeeder.SeedFromConfigAsync(
        app.Services,
    ...
    if (app.Environment.IsDevelopment())
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "Buildix API v1");
            options.DocumentTitle = "Buildix API Documentation";
