...
public class UnitOfWork : IUnitOfWork
{
    ...
    {
        // Reentrancy guard. If a transaction is already open on this UnitOfWork
        // — i.e. this is a nested ExecuteInTransactionAsync called from inside
        // another one — join the existing transaction instead of opening a
        // second one. Without this, the inner call's BeginTransactionAsync would
        // overwrite _transaction and its Commit would dispose + null it, leaving
        // the outer scope to commit an already-disposed transaction. The
        // outermost call owns begin / commit / rollback / concurrency-retry;
        // a nested call just contributes its writes to that same unit of work.
        if (_transaction != null)
        {
