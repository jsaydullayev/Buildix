...
public static class AuditActions
{
    ...
    // Bulk physical inventory count (Инвентаризация) — one event per product
    // whose counted figure differed from the system figure.
    public const string Stocktake = "Stocktake";

    // ── Fraud-sensitive sale mutations ────────────────────────────────────
    // Every value-changing action on an existing sale gets its OWN action so
    // the security journal can isolate the operations most abused for internal
    // theft (return a line & pocket the cash, drop a price / add a discount on
    // a debt sale, convert a paid-able sale to debt). Deliberately NOT reusing
    // the generic Delete action — that feeds the bulk-delete burst detector and
    // must stay reserved for hard deletes.
    public const string Return = "Return";
    public const string Discount = "Discount";
    public const string PriceOverride = "PriceOverride";
    public const string MarkDebt = "MarkDebt";
